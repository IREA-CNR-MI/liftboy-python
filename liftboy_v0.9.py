...
def make_path_simple(path):
    outString = path
    for result in re.findall(r"\[.*?\]", path):
    ...
    for itemKey in itemList.keys():
        ...
        for detailKey in itemDetails.keys():
            ...
            if (detailKey != "path" and detailKey != "id" and detailKey != "element_Id") or liftNeeded:
                ...
                detail.text = make_path_simple(itemDetails.get("path"))
        ...
        if len(node.xpath(make_path_relative(maxCommonPath, itemDetails.get("path")), namespaces=nsm)) > 0:
            ...
            if '@' in itemDetails.get("path")[itemDetails.get("path").rfind('/'):]:
                ...
                if str(node.xpath(make_path_relative(maxCommonPath, itemDetails.get("path")), namespaces=nsm)[0]) != "":
                    theValue = str(
                        node.xpath(make_path_relative(maxCommonPath, itemDetails.get("path")), namespaces=nsm)[0])
            else:
...
'''
variable "elementList" contains the dictionary containing the descriptions of the elements defined in the template
e.g.:
    resp: {
        'id': 'resp', 
        ...
        'produces': {
            'resp_1': {
                'id': 'resp_1', 
                'element_Id': 'resp', 
                'path': 'gmd:citedResponsibleParty/gmd:CI_ResponsibleParty/gmd:contactInfo/gmd:CI_Contact/gmd:address/gmd:CI_Address/gmd:electronicMailAddress/gco:CharacterString', 
                'datatype': 'autoCompletion', 
                'fixed': 'false', 
                'useCode': '', 
                'useURN': '', 
                'outIndex': '2', 
...
# debug: number and name of the elements in the template
...
outputTree.write(inputFile[:inputFile.rfind(".")]+".ediml")
...
inputTree.write(inputFile[:inputFile.rfind(".")]+"_transformed.ediml")